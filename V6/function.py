...
def ukaz_RS_Data(odkud,datahex,hexpara):
   for i in odkud:
      if (i.para == hexpara):
         if (i.para == "157C"):
            data_ven = int(datahex,16)
         ...
         i.value = data_ven
         #return ()
         print (i.para,"-",str(i.popis).ljust(45),"-",str(i.value).ljust(7),"-",str(i.send).ljust(6),"-",i.LastValue,"-", i.idx)
         #print (i.para,"-value-",val.ljust(7),"-Last Value-",lastval.ljust(7),"-", i.popis,)
...
def run_file(file_path):
    try:
        ...
        print(f"The output of {file_path} is:\n{result.stdout}")
    except subprocess.CalledProcessError as e:
        print(f"An error occurred while trying to run {file_path}: {str(e)}")
        print(f"The error output is:\n{e.output}")
    return result
