...
if (argLen > 1):
    print("byl zadany parametr:", sys.argv[1])
    if (sys.argv[1] == "--setup") or (sys.argv[1] == "-s"):
        print("byl zadany parametr setup, spoustim nastaveni")
        print ("spustenim se vymaže existujic soubor setup.dat a vytvoří se novy. Chcete Pokracovat? A/N: ")
        RunSetupFile = os.path.join(dirname, 'setup.py')
        r = run_file(RunSetupFile)
        print("spousteni souboru skončilo:", r)
